#!/usr/bin/env bash
# MONAI SwinViT scratch · same protocol as MONAI row, FDG init = random (no Tang SSL).
# FDG 3GPU bs=6 100ep → PSMA fs50/10/5 9-fold (waves of 3) → TEST20 → fc70 → fs0 → FDG TEST.
#
#   bash ICLR2026/run/run_aligned_monai_scratch_9fold_pipeline_bg.sh
set -euo pipefail
...
FOLDS_CSV="${TASK1_MONAI_SCRATCH_FOLDS:-0,1,2,3,4,5,6,7,8}"
GPUS="${TASK1_CUDA_VISIBLE_DEVICES:-0,1,3}"
POLL="${TASK1_CHAIN_POLL_SEC:-30}"
IDLE_MIB="${TASK1_GPU_IDLE_MEM_MIB:-2048}"
PID_FILE="${VIS}/monai_scratch_9fold_pipeline.pid"
LOG="${VIS}/nohup_monai_scratch_9fold_pipeline.log"
LAST_FDG="${VIS}/monai_scratch_fdg_LAST_STAMP.txt"
DONE_MARK="${VIS}/TASK1_MONAI_SCRATCH_9FOLD_DONE.txt"

mkdir -p "${VIS}"
if [[ -f "${PID_FILE}" ]]; then
  old="$(tr -d '[:space:]' < "${PID_FILE}" 2>/dev/null || true)"
  if [[ -n "${old}" && "${old}" != "$$" ]] && kill -0 "${old}" 2>/dev/null; then
    echo "[monai-scratch] already running pid=${old}"
    exit 0
  fi
fi
echo $$ > "${PID_FILE}"
...
echo "[monai-scratch] $(date '+%F %T') start pid=$$ folds=${FOLDS_CSV} gpus=${GPUS}"

export TASK1_BASE="${DATA}"
export TASK1_ALIGN_BOARD_JSON="${BOARD}"
export TASK1_BOARD_METHOD=monai_scratch
export TASK1_MAE_FOUNDATION_KIND=none
export TASK1_MAE_FREEZE_ENCODER_EPOCHS=0
export TASK1_MAE_BACKBONE_LR_MULT=1.0
...
m = (b.get("methods") or {}).get("monai_scratch") or {}
st = m.get("${1}") or {}
v = st.get("${2}")
if v is None:
    print("")
...
_wait_gpus_idle() {
  echo "[monai-scratch] wait GPUs ${GPUS} idle (<${IDLE_MIB} MiB)"
  while ! _gpus_idle >/dev/null; do
    echo "[monai-scratch] GPUs busy… $(TZ=Asia/Shanghai date +%H:%M:%S)"
    sleep "${POLL}"
  done
  sleep 15
  while ! _gpus_idle >/dev/null; do
    echo "[monai-scratch] GPUs busy again… $(TZ=Asia/Shanghai date +%H:%M:%S)"
    sleep "${POLL}"
  done
  echo "[monai-scratch] GPUs idle"
}
...
repo = Path(os.environ["REPO"]) / "runs" / stamp / "monai"
out = []
for f in folds:
    d = repo / f"fold{f}"
...
if [[ -z "${FDG_STAMP}" ]]; then
  FDG_STAMP="$(TZ=Asia/Shanghai date +%Y%m%d_%H%M%S)_iclr2026_monai_scratch_fdg_swinvit_gpu013_bs6_tr70_val10_100ep"
fi
FDG_BEST="${REPO}/runs/${FDG_STAMP}/best_seg_fdg_monai.pth"
FDG_LATEST="${REPO}/runs/${FDG_STAMP}/latest_seg_fdg_monai.pth"
CNAME="monai_fdg_${FDG_STAMP}"

if [[ ! -f "${FDG_BEST}" && ! -f "${FDG_LATEST}" ]]; then
  _wait_gpus_idle
  _disarm
  _board "{\"methods\":{\"monai_scratch\":{\"fdg_pretrain\":{\"status\":\"running\",\"stamp\":\"${FDG_STAMP}\",\"bs\":6,\"bs_note\":\"global 2×3GPU\",\"total_epochs\":100,\"note\":\"scratch → FDG 100ep\"}}},\"updated_note\":\"MONAI scratch FDG running\"}"
  ...
  echo "[monai-scratch] waiting FDG container ${CNAME}"
  ...
  while docker ps --format '{{.Names}}' | grep -qx "${CNAME}"; do
    echo "[monai-scratch] FDG still running $(TZ=Asia/Shanghai date +%H:%M:%S)"
    sleep "${POLL}"
  done
  _arm
fi
...
_board "{\"methods\":{\"monai_scratch\":{\"fdg_pretrain\":{\"status\":\"done\",\"stamp\":\"${FDG_STAMP}\",\"best_ckpt\":\"${FOUNDATION}\",\"bs\":6,\"bs_note\":\"global 2×3GPU\",\"total_epochs\":100,\"note\":\"scratch → FDG 100ep DONE\"}}},\"updated_note\":\"MONAI scratch FDG done\"}"
echo "[monai-scratch] FDG ckpt=${FOUNDATION}"
...
_run_fewshot() {
  ...
  if [[ -z "${stamp}" ]]; then
    stamp="$(TZ=Asia/Shanghai date +%Y%m%d_%H%M%S)_iclr2026_monai_scratch_psma_fs${n}_from_fdg_seg_f258_gpu013"
  fi
  local remain
  remain="$(_remaining_folds "${stamp}")"
  if [[ -n "${remain}" ]]; then
    echo "[monai-scratch] fs${n} train folds=${remain} stamp=${stamp}"
    export TASK1_NNUNET_RESULTS_STAMP_NAME="${stamp}"
    _disarm
    _board "{\"methods\":{\"monai_scratch\":{\"${stage}\":{\"status\":\"running\",\"stamp\":\"${stamp}\",\"note\":\"9fold · fs${n} · remaining ${remain}\"}}},\"updated_note\":\"MONAI scratch fs${n} 9fold\"}"
    TASK1_FEWSHOT_N="${n}" \
    TASK1_PSMA_BOARD_STAGE="${stage}" \
    TASK1_BOARD_METHOD=monai_scratch \
    TASK1_MONAI_FDG_SEG_CKPT="${FOUNDATION}" \
    ...
    echo "[monai-scratch] fs${n} all folds present stamp=${stamp}"
  fi
  echo "[monai-scratch] fs${n} TEST20 9fold"
  _disarm
  METHOD=monai_scratch STAMP="${stamp}" \
    TASK1_FEWSHOT_N="${n}" \
    TASK1_PSMA_BOARD_STAGE="${stage}" \
...
if pgrep -af 'run_monai_scratch_psma_fc70_from_fdg_seg_bg.sh|iclr2026_monai_scratch_psma_fc70' 2>/dev/null \
    | grep -Ev 'pgrep|queue_keeper|gpu_idle_queue' | grep -q .; then
  echo "[monai-scratch] fc70 already running (idle/other) — skip duplicate launch"
elif [[ "${fc70_st}" != "done" || -z "${fc70_mean}" ]]; then
  FC70_STAMP="$(_board_get psma_fc70 stamp)"
  [[ -n "${FC70_STAMP}" ]] || FC70_STAMP="$(TZ=Asia/Shanghai date +%Y%m%d_%H%M%S)_iclr2026_monai_scratch_psma_fc70_from_fdg_seg_gpu0"
  echo "[monai-scratch] fc70 stamp=${FC70_STAMP}"
  _disarm
  TASK1_BOARD_METHOD=monai_scratch \
  ...
  TASK1_PSMA_FC70_GPU=0 \
    bash "${CTRL}/ICLR2026/run/run_monai_scratch_psma_fc70_from_fdg_seg_bg.sh"
  _arm
else
  echo "[monai-scratch] fc70 already done"
fi

# ---------- PSMA fs0 / FDG TEST ----------
_disarm
METHOD=monai_scratch TASK1_TEST_SKIP_DONE=0 TASK1_CUDA_VISIBLE_DEVICES=0 \
...
echo "[monai-scratch] ALL DONE $(date '+%F %T')"
