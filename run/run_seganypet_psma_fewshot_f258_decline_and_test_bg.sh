#!/usr/bin/env bash
# SegAnyPET PSMA f258: resume from base ep until val-Dice decline, then TEST20.
set -euo pipefail
...
SEG_STAMP="${SEG_STAMP:?need SEG_STAMP}"
...
SEG_CODE="${ROOT}/ICLR2026/third_party/SegAnyPET/code"
SEG_PIP="${ROOT}/ICLR2026/third_party/seganypet_pip"
...
PIPE_LOG="${VIS}/nohup_seganypet_fs${FEWSHOT_N}_decline_${SEG_STAMP}.log"
...
_start_monitor() {
  local fold="$1"
  local log="${VIS}/nohup_decline_mon_seganypet_fs${FEWSHOT_N}_f${fold}.log"
  nohup python3 "${MON}" \
    --method seganypet \
    ...
    --val-every "${VAL_EVERY}" \
    >"${log}" 2>&1 &
  echo $! > "${VIS}/decline_mon_seganypet_fs${FEWSHOT_N}_f${fold}.pid"
}

python3 "${ROOT}/ICLR2026/scripts/iclr2026_aligned_fdg_fs50_board.py" \
  --board "${BOARD}" --no-plot \
  --patch-json "{\"methods\":{\"seganypet\":{\"${BOARD_STAGE}\":{\"status\":\"running\",\"stamp\":\"${SEG_STAMP}\",\"phase\":\"decline\",\"note\":\"resume@${BASE_EP} until val Dice decline\"}}},\"updated_note\":\"SegAnyPET fs${FEWSHOT_N} decline resume\"}" || true
...
for fold in "${_sgf[@]}"; do
  ...
  cname="seganypet_fs${FEWSHOT_N}_f${fold}_${SEG_STAMP}"
  seg_cnames+=("${cname}")
  ...
  nohup docker run --rm \
    ...
    -e PYTHONPATH="${SEG_PIP}:${SEG_CODE}:${ROOT}/ICLR2026/scripts" \
    -v "${ROOT}:${ROOT}" -v "${DATA}:${DATA}" \
    -w "${SEG_CODE}" --shm-size=8g "${IMAGE}" \
    python3 "${ROOT}/ICLR2026/scripts/seganypet_fewshot_finetune.py" \
      --data-root "${fold_data}" \
      --checkpoint "${SEG_CKPT}" \
      --out-dir "${out_dir}" \
      --epochs "${MAX_EP}" \
      --batch-size 2 --accumulation-steps 4 --num-workers 6 \
...
while true; do
  ...
  echo "[seg-decline] finished=${done}/${#seg_cnames[@]}"
  [[ "${done}" -ge "${#seg_cnames[@]}" ]] && break
  sleep 60
done

TASK1_PSMA_BOARD_STAGE="${BOARD_STAGE}" TASK1_FEWSHOT_N="${FEWSHOT_N}" \
  STAMP="${SEG_STAMP}" bash "${ROOT}/ICLR2026/run/run_eval_seganypet_psma_test20_f258_bg.sh"

python3 "${ROOT}/ICLR2026/scripts/iclr2026_aligned_fdg_fs50_board.py" \
  --board "${BOARD}" --no-plot \
  --patch-json "{\"methods\":{\"seganypet\":{\"${BOARD_STAGE}\":{\"status\":\"done\",\"stamp\":\"${SEG_STAMP}\",\"test_invalidated\":false,\"phase\":null,\"note\":\"TEST20 DONE · 3/3\"}}},\"updated_note\":\"SegAnyPET fs${FEWSHOT_N} TEST20 done\"}" || true

echo "[seg-decline] ALL DONE fs${FEWSHOT_N} ${SEG_STAMP}"
