...
def _prompts_from_properties(properties):
    """DpDNet prompt codes (lymp/mela/lung/brea). Autopet preprocess omits ``type``."""
    ...
    def one(prop):
        ...
        if isinstance(prop, dict):
            cid = str(prop.get("case_identifier") or "")
        code = cid[:4] if len(cid) >= 4 else "lymp"
        return [code]

    if isinstance(properties, dict):
        return [one(properties)]
    return [one(p) for p in properties]


def _network_forward(network, data, prompt):
    n = network
    if hasattr(n, "module"):
        n = n.module
    if isinstance(n, OptimizedModule):
        n = n._orig_mod
    if "prompt" in inspect.signature(n.forward).parameters:
        return network(data, prompt)
    return network(data)
...
except ImportError:  # optional; only used in commented FLOPs debug
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if self.is_ddp:
                ...
                # STUNet_prompt routes by cancer-type; unused branch params need this under DDP
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        prompt = _prompts_from_properties(properties)
        ###################################################################
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            ###################################################################
            output = _network_forward(self.network, data, prompt)
    ...
    def on_epoch_end(self):
        ...
        ema = self.logger.my_fantastic_logging['ema_fg_dice'][-1]
        try:
            ema_ok = float(ema) == float(ema) and abs(float(ema)) != float('inf')
        except (TypeError, ValueError):
            ema_ok = False
        try:
            best_ok = self._best_ema is not None and float(self._best_ema) == float(self._best_ema)
        except (TypeError, ValueError):
            best_ok = False
        if ema_ok and (self._best_ema is None or (not best_ok) or ema > self._best_ema):
            self._best_ema = ema
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.keys()):
                ...
                if isinstance(properties, dict):
                    prompt = _prompts_from_properties(properties)[0]
