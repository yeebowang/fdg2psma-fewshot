...
class nnUNetLogger(object):
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'mean_fg_dice':
            ...
            prev = None
            ema_hist = self.my_fantastic_logging['ema_fg_dice']
            if epoch > 0 and len(ema_hist) >= epoch:
                prev = ema_hist[epoch - 1]
            if not _finite(value):
                new_ema_pseudo_dice = prev if _finite(prev) else float('nan')
            elif _finite(prev):
                new_ema_pseudo_dice = float(prev) * 0.9 + 0.1 * float(value)
            else:
