...
def unpack_dataset(folder: str, unpack_segmentation: bool = True, overwrite_existing: bool = False,
    ...
    If blosc2 ``.b2nd`` files are present (modern nnU-Net / Dataset228 style), skip unpacking.
    Unpacking to ``.npy`` forces full-volume mmap IO and is much slower for patch training.
    """
    try:
        names = os.listdir(folder)
    except FileNotFoundError:
        return
    if any(n.endswith(".b2nd") and not n.endswith("_seg.b2nd") for n in names):
        print(f"[unpack_dataset] skip: blosc2 .b2nd present under {folder}")
        return
...
def get_case_identifiers(folder: str) -> List[str]:
    """
    Prefer blosc2 ``.b2nd`` case ids (nnU-Net default); fall back to ``.npz``.
    """
    names = os.listdir(folder)
    b2nd = [
        i[:-5]
        for i in names
        if i.endswith(".b2nd") and (not i.endswith("_seg.b2nd")) and ("segFromPrevStage" not in i)
    ]
    if b2nd:
        return b2nd
    case_identifiers = [
        i[:-4] for i in names if i.endswith("npz") and (i.find("segFromPrevStage") == -1)
    ]
