...
try:
    from nnunetv2.training.nnUNetTrainer.STUNetTrainer import STUNetTrainer_small  # noqa: F401
except ImportError:
    STUNetTrainer_small = None
try:
    from nnunetv2.training.nnUNetTrainer.STUNetTrainer import (  # noqa: F401
        STUNetTrainer_small_prompt,
        STUNetTrainer,
    )
except ImportError:
    STUNetTrainer_small_prompt = None
    STUNetTrainer = None
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        if os.environ.get("TASK1_DPDNET_SKIP_FINAL_VAL", "0") == "1":
            print("[train] skip perform_actual_validation (TASK1_DPDNET_SKIP_FINAL_VAL=1)")
            return
