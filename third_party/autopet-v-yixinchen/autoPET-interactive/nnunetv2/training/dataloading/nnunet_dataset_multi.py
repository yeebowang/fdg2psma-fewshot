...
class nnUNetDatasetMultiTask(nnUNetBaseDataset):
    def __init__(self, folder: str, identifiers: List[str] = None,
                 folder_with_segs_from_previous_stage: str = None):
    ...
    def load_case(self, identifier):
        ...
        seg_org_b2nd_file = join(self.source_folder, identifier + '_seg_org.b2nd')
        seg_org = blosc2.open(urlpath=seg_org_b2nd_file, mode='r', dparams=dparams, mmap_mode='r')

        properties = load_pickle(join(self.source_folder, identifier + '.pkl'))
        return data, seg, seg_org, properties
    ...
    def load_case_with_clicks(self, identifier):
        ...
        click_json = join(self.source_folder, os.pardir, "clicks", identifier + '_clicks.json')
        click_json = load_json(click_json)
    ...
    def save_case(
        ...
        blosc2.asarray(np.ascontiguousarray(org), urlpath=output_filename_truncated + '_seg_org.b2nd',
                       chunks=chunks_org, blocks=blocks_org, cparams=cparams, mmap_mode='w+')
        write_pickle(properties, output_filename_truncated + '.pkl')
...
file_ending_dataset_mapping = {
    ...
    'b2nd': nnUNetDatasetMultiTask
}


def infer_dataset_class(folder: str) -> Union[Type[nnUNetDatasetMultiTask], Type[nnUNetDatasetNumpy]]:
