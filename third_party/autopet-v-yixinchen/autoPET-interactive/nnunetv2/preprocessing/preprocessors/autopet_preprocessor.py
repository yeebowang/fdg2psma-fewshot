...
class autoPetPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], org: Union[np.ndarray, None], properties: dict,
        ...
        if seg is not None:
            ...
            assert data.shape[1:] == org.shape[1:], "Shape mismatch between image and organ segmentation. Please fix your dataset and make use of the --verify_dataset_integrity flag to ensure everything is correct"
            seg = np.copy(seg)
            org = np.copy(org)
        ...
        if seg is not None:
            ...
            org = org.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
        ...
        _, org, _ = crop_to_nonzero(data, org)
        ...
        org = configuration_manager.resampling_fn_seg(org, new_shape, original_spacing, target_spacing)
        if self.verbose:
        ...
        if has_seg:
            ...
            org = self.modify_seg_fn(org, plans_manager, dataset_json, configuration_manager)
        if np.max(seg) > 127:
        ...
        if np.max(org) > 127:
            org = org.astype(np.int16)
        else:
            org = org.astype(np.int8)
        return data, seg, org, properties
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], plans_manager: PlansManager,
        ...
        if seg_file is not None:
            ...
            org, _ = rw.read_seg(seg_file.replace("labelsTr", "labelsTr_organs"))
        else:
            seg = None

        if org is None:
            raise RuntimeError(
                f'No organ segmentation found for {seg_file}. Please ensure that the organ segmentation is available '
            )

        if self.verbose:
            print(seg_file)
        data, seg, org, data_properties = self.run_case_npy(data, seg, org, data_properties, plans_manager, configuration_manager,
                                      dataset_json)
        return data, seg, org, data_properties

    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
                      plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                      dataset_json: Union[dict, str]):
        data, seg, org, properties = self.run_case(image_files, seg_file, plans_manager, configuration_manager, dataset_json)
        data = data.astype(np.float32, copy=False)
        seg = seg.astype(np.int16, copy=False)
        org = org.astype(np.int16, copy=False)
        # print('dtypes', data.dtype, seg.dtype)
        block_size_data, chunk_size_data = nnUNetDatasetMultiTask.comp_blosc2_params(
            data.shape,
            tuple(configuration_manager.patch_size),
            data.itemsize)
        block_size_seg, chunk_size_seg = nnUNetDatasetMultiTask.comp_blosc2_params(
            seg.shape,
            tuple(configuration_manager.patch_size),
            seg.itemsize)
        block_size_org, chunk_size_org = nnUNetDatasetMultiTask.comp_blosc2_params(
            org.shape,
            tuple(configuration_manager.patch_size),
            org.itemsize)

        nnUNetDatasetMultiTask.save_case(data, seg, org, properties, output_filename_truncated,
                                      chunks=chunk_size_data, blocks=block_size_data,
                                      chunks_seg=chunk_size_seg, blocks_seg=block_size_seg,
                                        chunks_org=chunk_size_org, blocks_org=block_size_org)
