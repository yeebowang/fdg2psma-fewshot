...
def load_pretrained_weights(network, fname, verbose=False):
    ...
    local_edit_expansion = saved_model.get('autopet_champion_to_localedit7')
    if local_edit_expansion is not None:
        if local_edit_expansion.get('format') != 'autopet_champion_to_localedit7_v1':
            raise RuntimeError(f"unsupported LocalEdit7 initialization: {local_edit_expansion}")
        ...
        missing = sorted(set(model_dict) - set(pretrained_dict))
        extra = sorted(set(pretrained_dict) - set(model_dict))
        mismatched = sorted(
            (key, tuple(pretrained_dict[key].shape), tuple(model_dict[key].shape))
            for key in set(model_dict) & set(pretrained_dict)
            if pretrained_dict[key].shape != model_dict[key].shape
        )
        if missing or extra or mismatched:
            raise RuntimeError(
                "LocalEdit7 champion initialization is not strictly compatible: "
                f"missing={missing[:10]} extra={extra[:10]} mismatched={mismatched[:10]}"
            )
        for key in local_edit_expansion['migrated_stem_keys']:
            if torch.count_nonzero(pretrained_dict[key][:, 2:]).item() != 0:
                raise RuntimeError(f"LocalEdit7 non-image stem channels are not fresh: {key}")
        for key in local_edit_expansion['fresh_lesion_head_keys']:
            if torch.count_nonzero(pretrained_dict[key]).item() != 0:
                raise RuntimeError(f"LocalEdit7 action head is not fresh: {key}")
        mod.load_state_dict(pretrained_dict, strict=True)
        print(
            "AUTOPET_LOCAL_EDIT7_CHAMPION_INIT_LOADED "
            f"keys={len(pretrained_dict)} fresh_inputs={local_edit_expansion['fresh_input_channels']}"
        )
        return
