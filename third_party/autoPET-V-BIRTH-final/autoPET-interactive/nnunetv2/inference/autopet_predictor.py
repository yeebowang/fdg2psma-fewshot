...
class autoPETPredictor(object):
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
                                 clicks: dict, point_width: float = 1.0,
        ...
        pos_clicks, neg_clicks = sparse_to_dense_point_nnInteractive(clicks.get("points", []), dct['data'].shape[1:],
                                                                        dct['data_properties'], sigma=point_width)
        
        # import napari
        # viewer = napari.Viewer()
        # viewer.add_image(dct['data'][0].numpy(), name='ct')
        # viewer.add_image(dct['data'][1].numpy(), name='pet')
        # viewer.add_image(pos_clicks.numpy(), name='pos_clicks', colormap='green', blending='additive')
        # viewer.add_image(neg_clicks.numpy(), name='neg_clicks', colormap='red', blending='additive')
        # napari.run()

        if self.verbose:
            print('predicting')
        data = torch.vstack((dct['data'], pos_clicks[None], neg_clicks[None]))
