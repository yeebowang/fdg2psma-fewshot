...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    # load images
    seg_ref, voxel_vol = nii2numpy(reference_file)
    seg_pred, voxel_vol = nii2numpy(prediction_file)

    false_neg_vol = false_neg_pix(seg_ref, seg_pred)*voxel_vol
    false_pos_vol = false_pos_pix(seg_ref, seg_pred)*voxel_vol
    ...
    for r in labels_or_regions:
        ...
        if tp + fp + fn == 0 or np.sum(seg_ref) == 0:
            results['metrics'][r]['Dice'] = np.nan
            results['metrics'][r]['IoU'] = np.nan
            results['metrics'][r]['FNvol'] = np.nan
        else:
            results['metrics'][r]['Dice'] = 2 * tp / (2 * tp + fp + fn)
            results['metrics'][r]['IoU'] = tp / (tp + fp + fn)
            results['metrics'][r]['FNvol'] = false_neg_vol
        results['metrics'][r]['FPvol'] = false_pos_vol
...
def compute_metrics_autopet(nii_gt_path, nii_pred_path):
    # main function
