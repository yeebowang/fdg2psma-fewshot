...
from utils.click_method import get_next_click3D_torch_3
...
parser.add_argument('--task_name', type=str, default='union_train_turbo_lqonly_uncrec')
...
click_methods = {
    'random': get_next_click3D_torch_3,
}
...
def interaction(self, sam_model, image_embedding, gt3D, num_clicks):
    ...
    output_preds = []  
    for num_unc in range(5):  
        points_input, labels_input = self.get_points(prev_masks, gt3D)
        low_res_masks, prev_masks = self.batch_forward(
            sam_model, image_embedding, gt3D, low_res_masks, points=[points_input, labels_input]
        )
        output_preds.append(prev_masks)
    
    stacked_preds = torch.stack(output_preds) 
    ensemble, uncertainty_map = compute_epistemic_uncertainty(stacked_preds)
    unc_threshold = torch.mean(uncertainty_map) 
    unc_high_mask = (uncertainty_map > unc_threshold)  
    rectified_seg = ensemble.bool() * (unc_high_mask == 0) & (ensemble == 0) * unc_high_mask 

    for num_click in range(num_clicks):
        points_input, labels_input = self.get_points(prev_masks, rectified_seg)  
        if num_click == random_insert or num_click == num_clicks - 1:
            low_res_masks, prev_masks = self.batch_forward(sam_model, image_embedding, rectified_seg, low_res_masks, points=None)
        else:
            low_res_masks, prev_masks = self.batch_forward(sam_model, image_embedding, rectified_seg, low_res_masks, points=[points_input, labels_input])
        loss = self.seg_loss(prev_masks, rectified_seg)
        return_loss += loss
    ...
    return prev_masks, return_loss
        ...
        for num_click in range(num_clicks):
            ...
            output_preds.append(prev_masks)
        stacked_preds = torch.stack(output_preds)
        ensemble, uncertainty_map = compute_uncertainty(stacked_preds)
        unc_threshold = torch.mean(uncertainty_map)
        unc_high_mask = (uncertainty_map>unc_threshold)
        rectified_label = gt3D.bool() * (unc_high_mask == 0) & (gt3D == 0) * unc_high_mask
        for pred in output_preds:
            loss1 = self.seg_loss(pred, gt3D)
            loss2 = self.seg_loss(pred, rectified_label)
