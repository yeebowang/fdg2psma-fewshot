...
def get_roi_from_subject(subject_canonical, meta_info, crop_transform, norm_transform):
    ...
    # torchio>=1.2 requires target_shape explicitly
    target_shape = getattr(crop_transform, "target_shape", None)
    if target_shape is None:
        # CropOrPad stores as _target_shape in some versions
        target_shape = getattr(crop_transform, "_target_shape", None)
    padding_params, cropping_params = crop_transform._compute_center_crop_or_pad(
        subject_canonical, target_shape
    )
