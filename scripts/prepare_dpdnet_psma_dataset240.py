#!/usr/bin/env python3
"""Build Dataset240 for DpDNet PSMA fewshot from Dataset228 + MAE fs50 9fold splits.

Same prompt pattern as Dataset239: rename ``psma_*`` → ``lymp_psma_*`` and inject
``type=['lymp','lymp']`` into properties pkl. ``splits_final.json`` keeps 9 folds
(indices 0..8) so training can use fold=2/5/8 directly.
"""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=Path(
            "/media/ybwang/data1/PSMA-CTRL/ICLR2026/data/splits_mae_psma_fewshot50_9fold"
        ),
    )
    ap.add_argument("--src-id", type=int, default=228)
    ap.add_argument("--dst-id", type=int, default=240)
    ...
    dst_name = f"Dataset{args.dst_id}_DpDNet_PSMA_2ch"
    ...
    for i in range(args.n_folds):
        sp = args.split_dir / f"fold{i}_nnunet.json"
        if not sp.is_file():
    ...
    for fold in folds:
        splits_out.append(
            {
                "train": [f"{pref}_{c}" for c in fold["train"]],
                "val": [f"{pref}_{c}" for c in fold["val"]],
            }
        )
    ...
    meta = {
        "dst": dst_name,
        "n_cases": len(cases),
        "n_folds": len(splits_out),
        "fold_sizes": [
            {"fold": i, "n_train": len(s["train"]), "n_val": len(s["val"])}
            for i, s in enumerate(splits_out)
        ],
        "prompt_prefix": pref,
