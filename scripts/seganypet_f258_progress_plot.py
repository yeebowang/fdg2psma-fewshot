#!/usr/bin/env python3
"""Progress plot for SegAnyPET fewshot50 f2/5/8."""
...
def _best(rows):
    ...
    for r in rows:
        ...
        if vd is None or vd != vd:
            continue
        vd = float(vd)
        # 0.0 usually means broken empty-pred eval; ignore until real scores exist
        if vd <= 1e-6:
            continue
...
def collect(run_root: Path, folds, ft_epochs: int):
    # Prefer patched aggregate / reeval when metrics.jsonl val was broken (0/NaN)
    agg_scores = {}
    for name in ("aggregate_val_dice_f258.json", "aggregate_val_dice_f258_reeval.json"):
        p = run_root / name
        if not p.is_file():
            continue
        try:
            ...
            continue
        vals = d.get("fold_best_dice") or d.get("fold_dice")
        if not vals:
            continue
        for fold, v in zip(d.get("folds", folds), vals):
            if v is None or v != v:
                continue
            vf = float(v)
            if vf > 1e-6:
                agg_scores[int(fold)] = vf
        if agg_scores:
            break

    jobs = []
    for fold in folds:
        rows = _load_jsonl(run_root / "seganypet" / f"fold{fold}" / "metrics.jsonl")
        ...
        best = _best(rows)
        if best is None and fold in agg_scores:
            best = agg_scores[fold]
        elif fold in agg_scores and (best is None or agg_scores[fold] > best):
            best = agg_scores[fold]
        mpath = run_root / "seganypet" / f"fold{fold}" / "metrics.jsonl"
        done = cur >= ft_epochs
        running = mpath.is_file() and (not done) and (time.time() - mpath.stat().st_mtime) < 300
        jobs.append(
            {"fold": fold, "cur": cur, "best": best, "done": done, "running": running, "avg_sec": avg, "rows": rows}
        )
    mean_fold = 7200.0
    for j in jobs:
        if j["avg_sec"]:
...
def render(state, out_png: Path):
    ...
    ax0.set_yticks([])
    ax0.set_title(f"[train] SegAnyPET jobs {state['n_done']}/{len(jobs)}", fontweight="bold")
...
def main():
    ...
    print(
        f"[seganypet-progress] ft={state['n_done']}/{len(state['jobs'])} ETA={_fmt_eta(state['eta_s'])} → {args.out_png}",
        flush=True,
    )
